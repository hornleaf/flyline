    ...
    T: Terminal,
{
        ...
        I: Iterator<Item = (u16, u16, &'a Cell)>,
    {
        ...
        let mut underline_color = Color::Reset;
        let mut modifier = Modifier::empty();
        // Track the *real* terminal column rather than the ratatui grid
        // coordinate.  A wide grapheme (e.g. CJK, emoji) occupies two terminal
        // columns while its grid coordinate only advances by one, so using
        // grid coordinates for relative moves drifts by one column per wide
        // character and corrupts the whole line.
        // Next output column relative to the line start, and the column where
        // the previous grapheme ended.  Both track *real* terminal columns
        // (wide graphemes advance by two).
        let mut terminal_col: u16 = 0;
        let mut last_output_col: u16 = 0;
        // Width of the last grapheme actually emitted.  Empty cells following
        // a wide grapheme are continuation cells: they are already covered
        // visually by the wide grapheme, so emitting a space there would shift
        // every following column by one.
        let mut last_emitted_width: u16 = 1;

        for (_, _, cell) in content {
            let symbol = cell.symbol();
            let symbol_width = symbol.cell_width() as u16;

            if symbol.is_empty() && last_emitted_width > 1 {
                // Continuation cell of a wide grapheme: nothing to emit.
                continue;
            }

            let output = if symbol.is_empty() { " " } else { symbol };
            let output_width = if symbol.is_empty() { 1 } else { symbol_width };

            let dx = terminal_col as i16 - last_output_col as i16;
            if dx > 0 {
            ...
            last_output_col = terminal_col + output_width;
            terminal_col = last_output_col;
            last_emitted_width = output_width;
        }
